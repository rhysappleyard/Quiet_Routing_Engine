...
if st.session_state.orig is not None:
    ...
    with st.spinner("Optimising quiet route..."):
        ...
        with col1:
            st.metric(label="Fast Route", value=f"{len_fast/1000:.1f} km", delta=format_time(int(fast_time)))
            st.metric(label="Average Noise on Fast Route", value=f"{fast_noise} dB", delta =f"+{(fast_noise - quiet_noise):.0f} dB")
        with col2:
