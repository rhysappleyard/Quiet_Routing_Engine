...
if st.session_state.orig is not None:
    ...
    with st.spinner("Optimising quiet route..."):
        ...
        with col1:
            ...
            st.metric(label="Average Noise on Fast Route", value=f"{fast_noise} dB")
        with col2:
            st.metric(label=f"{k_label} Route", value=f"{len_quiet/1000:.1f} km", delta=format_time(int(quiet_time)), delta_color="inverse")
