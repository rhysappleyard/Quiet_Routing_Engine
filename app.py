...
if st.session_state.orig is not None:
    ...
    with st.spinner("Optimising quiet route..."):
        ...
        with col1:
            ...
            st.metric(label="Average Noise", value=f"{int(fast_noise)} dB", delta =f"+{(fast_noise - quiet_noise):.0f} dB", delta_color="inverse")
        with col2:
            st.metric(label=f"{k_label} Route", value=f"{len_quiet/1000:.1f} km", delta=quiet_time_str, delta_color="off")
            st.metric(label=f"Average Noise", value=f"{int(quiet_noise)} dB", delta =f"{(quiet_noise - fast_noise):.0f} dB", delta_color="inverse")
